...
public interface SwaggerMovementController {
    ...
    @Operation(summary = "Criar uma movimentação", description = "Registra uma nova movimentação de produto no sistema")
    ...
    @Operation(summary = "Listar todas as movimentações", description = "Retorna uma lista paginada de movimentações")
    ResponseEntity<Page<MovementResponseDTO>> getAll(
            @Parameter(description = "Informações de paginação e ordenação")
            Pageable pageable
    );

    @Operation(summary = "Buscar movimentação por ID", description = "Retorna uma movimentação pelo seu ID")
    ResponseEntity<MovementResponseDTO> getById(
            @Parameter(description = "ID da movimentação a ser buscada", example = "1", required = true)
            Long id
    );

    @Operation(summary = "Atualizar movimentação", description = "Atualiza os dados de uma movimentação pelo seu ID")
    ResponseEntity<MovementResponseDTO> update(
            ...
            Long id,
            @Parameter(description = "Dados atualizados da movimentação", required = true)
            MovementRequestDTO data
    );

    @Operation(summary = "Deletar movimentação", description = "Remove uma movimentação do sistema pelo seu ID")
